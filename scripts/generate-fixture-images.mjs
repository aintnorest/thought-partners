...
writeFileSync(
  join(root, "public/images/demo/index.json"),
  `${JSON.stringify(manifest, null, 2)}\n`,
);

const sections = manifest
  .map(
    (recipe) => `<section id="${esc(recipe.id)}" aria-labelledby="${esc(recipe.id)}-title">
...
writeFileSync(
  join(root, "public/demo-images.html"),
  `<!doctype html>
...
console.log(
  `Generated ${imageCount} illustrations for ${manifest.length} recipes. Gallery: /demo-images.html`,
);

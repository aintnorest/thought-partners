...
- **WatchMePanel:** Camera-assisted coaching sheet layered into the walkthrough, not a separate page. Shows camera preview, current `doneWhen`/`-- watch:` target, one latest fix/readiness card, and a stop control. Default visual state is calm/silent: Raised Charcoal surface, Whisper Warm border, tiny pulsing Ember camera dot only while actively sampling frames. Status uses semantic rails: Herb Green for ready, Saffron Amber for coach/check, Brick Red for visible issue.

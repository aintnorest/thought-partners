...
export const useStore = create<StoreState>()((set, get) => ({
  ...
  showFix: (message) => {
    ...
    set((current) => ({
      ...
      cards: stepId
        ? [
            ...current.cards.filter((card) => card.kind !== "watch_fix" || card.stepId !== stepId),
            { kind: "watch_fix", stepId, line: message },
          ]
        : current.cards,
    }));
  },
      ...
      cards: stepId
        ? [
            ...current.cards.filter(
              (card) => card.kind !== "watch_ready" || card.stepId !== stepId,
            ),
            { kind: "watch_ready", stepId, line: cue },
          ]
        : current.cards,
    }));
  },
